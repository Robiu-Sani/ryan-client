    ...
    <div className="group relative border cursor-pointer flex md:grid bg-white p-2 md:p-0 md:pb-10 overflow-hidden shadow-md hover:shadow-lg transition duration-300">
      ...
      <Link href={"/product-details"} className="">
        ...
        {/* <Tooltip showArrow={true} content="Add to Cart" className='text-black bg-white'>  
          ...
          <Tooltip showArrow={true} content="Add to Compare" className='text-black bg-white'>
          ...
          <Tooltip showArrow={true} content="Add to Favourite" className='text-black bg-white'>
          ...
          <Tooltip showArrow={true} content="Quick View" className='text-black bg-white'>
          <button onClick={onOpen} className="bg-white h-7 w-7 text-black rounded-full shadow hover:bg-[#666] hover:text-white">
      ...
      <div className="p-2 text-left md:text-center relative">
            <h3 className=" text-slate-900 font-semibold text-sm">{product.name}</h3>
        {/* <p className="text-[14px] text-gray-600">{product.description}</p> */}
        <div className="mt-2 font-bold text-slate-900 text-sm md:text-base">Tk {product.price}</div>
        <p className="text-xs font-semibold text-purple-700">{product.offer}</p>
      ...
      <Modal className="relative top-14 md:top-10 rounded-none" isOpen={isOpen} size="4xl" onOpenChange={onOpenChange}>
          ...
          <div className="flex lg:flex-row flex-col justify-center">
            ...
            >
              <div className="relative flex gap-5">
                ...
                <div className="small-images w-[20%]" style={styles.smallImagesContainer}>
                ...
                <div className="large-image w-[95%]" style={styles.largeImageContainer}>
                  <img src={selectedImage} alt="Selected" className="" style={styles.largeImage} />
                </div>
                <p className="absolute bottom-[-45px] text-[9px] w-[87%]">
                  N.B. Image may differ with actual products layout, color, size & dimension. No claim will be
                  accepted for image mismatch.
                </p>
              </div>
            </div>
            ...
            <div className="lg:w-[50%] w-full mt-10 md:mt-0 md:p-3">
              <h2 className="text-[20px] font-semibold text-gray-800 max-w-[90%]">
                Dell Vostro 15 3530 Intel Core i3 1305U 8GB RAM, 512GB SSD 15.6 Inch FHD Display Titan Grey Laptop
              </h2>
              <div className="flex justify-start items-center gap-2">
                <div className="flex justify-start items-center gap-1">
                  {[...Array(totalStars)].map((star, index) => {
                    return (
                      <span key={index} className="my-3" style={styles.star}>
                        {index < rating ? <FaStar className="text-sm" /> : <PiStarThin className="text-sm" />}
                      </span>
                    );
                  })}
                </div>
              ...
              <p className="text-sm mb-1 text-gray-700">Product Id: 101.04.754.15</p>
              <div className="bg-[#F2F2F2] p-2 w-[200px]">
                <p className="text-sm">Special Price</p>
                <p className="text-[20px] font-semibold text-gray-900">Tk 11,000</p>
              </div>
              <Tooltip
                placement="top"
                showArrow={true}
              ...
              >
                <p className="text-blue-500 font-semibold text-sm mb-3 mt-1">Save Tk 10,000 on online order</p>
              </Tooltip>
              ...
              <div className="mb-4">
                ...
                <p className="text-gray-900 text-[13px] font-semibold">EMI: Tk 27,035</p>
              </div>
...
<div className='flex md:hidden items-center w-[65%] mx-auto  gap-2 absolute bottom-3 right-[-5px]'>
        <div onClick={handleAddToWishlist} className='bg-[#72BF44] text-sm font-bold py-1 px-2'>Add to Cart</div>
        <Tooltip showArrow={true} content="Add to Compare" className='text-black bg-[#f2f2f2]'>
          <button className="bg-[#f2f2f2] h-7 w-7 text-black rounded-md shadow hover:bg-[#666] hover:text-white">
            <GoArrowSwitch className=" text-base w-fit mx-auto" />
          </button>
          </Tooltip>
          <Tooltip showArrow={true} content="Add to Favourite" className='text-black bg-[#f2f2f2]'>
          <button className="bg-[#f2f2f2] h-7 w-7 text-red-600 rounded-md shadow hover:bg-[#666] ">
            <FaHeart className=" text-base w-fit mx-auto" />
          </button>
          </Tooltip>
          <Tooltip showArrow={true} content="Quick View" className='text-black bg-[#f2f2f2]'>
          <button onClick={onOpen} className="bg-[#f2f2f2] h-7 w-7 text-black rounded-md shadow hover:bg-[#666] hover:text-white">
            <FaEye className=" text-lg w-fit mx-auto" />
          </button>
          </Tooltip>
      </div>

<div onClick={handleAddToWishlist} className='bg-[#cccccc] hidden md:grid absolute bottom-[-1px] mt-5 w-full text-center text-black font-bold text-sm py-2'>Add to Cart</div>
